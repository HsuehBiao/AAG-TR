...
% Impro_HBNC: 改进的层次二分邻居聚类算法，用于优化聚类结果。
% 输入:
%   X0: 输入数据矩阵，大小为 n x m，其中 n 是样本数量，m 是特征维度。
%   label_pre: 初始的类别标签，大小为 n x 1。
%   object_pre: 初始的类别目标值，大小为 class x 1。
% 输出:
%   label: 更新后的类别标签，大小为 n x 1。
%   object: 更新后的类别目标值，大小为 class x 1。
%   theta: 更新后的类别中心点，大小为 class x m。
%   num_class: 更新后的每个类别的样本数量，大小为 class x 1。
%   class: 更新后的类别数量。
flag = 1;
n = size(X0,1);
iter = 1;
