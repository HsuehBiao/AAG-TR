...
% Pre_HBNC: 对输入数据进行预处理的函数，用于生成类别标签、目标对象、类别中心、类别数量等信息。
% 输入:
%   X0: 输入数据矩阵，大小为 n x m，其中 n 是样本数量，m 是特征维度。
% 输出:
%   label: 每个样本的类别标签，大小为 n x 1。
%   object: 每个类别的目标对象，表示类内样本到类别中心的距离之和。
%   theta: 每个类别的中心点，大小为 num_class x m。
%   num_class: 每个类别的样本数量，大小为 num_class x 1。
%   class: 最终的类别数量。
flag = 1;
n = size(X0,1);
label = zeros(n,1);
